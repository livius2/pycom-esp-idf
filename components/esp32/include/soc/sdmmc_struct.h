...
typedef volatile struct {
    ...
    union {
        struct {
            ...
            uint32_t div_factor_l: 4;       ///< clock division factor, low level
            uint32_t div_factor_h: 4;       ///< clock division factor, high level
            uint32_t div_factor_p: 4;       ///< should be equal to div_factor_h
        };
        uint32_t val;
    } clock;
} sdmmc_dev_t;
